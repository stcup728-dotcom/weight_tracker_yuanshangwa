...
if len(df) > 0:
    ...
    fig.update_layout(legend_title_text="", xaxis=dict(title="日期", tickformat="%Y/%-m/%-d", rangeslider=dict(visible=True)),
...
if len(df) > 0:
    ...
    fig2 = px.line(last, x="date", y="weight_jin", color="name", markers=True,
                   labels={"date":"日期","weight_jin":"体重(斤)","name":"姓名"})
    fig2.update_layout(legend_title_text="", xaxis=dict(title="日期", tickformat="%Y/%-m/%-d"),
                       yaxis=dict(title="体重(斤)"), hovermode="x unified",
                       plot_bgcolor="white", paper_bgcolor="white")
